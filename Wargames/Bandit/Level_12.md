...
# --- 삽질 로그(교훈) ---
# (0) sudo apt-get install binwalk → "sudo must be owned by uid 0 and have the setuid bit set"
#     → bandit엔 root 없음 + sudo가 setuid 아님 = 도구 설치 불가. '있는 것으로만' 풀어야 함.
# (1) 'xxr' 오타 → command not found. 의도한 건 xxd.
# (2) xxd 출력 파일 지정을 플래그로 오해:
#       xxd -d data.txt          → '-d'는 decimal offset 표시일 뿐, 역변환 아님 → 여전히 hexdump
#       xxd -d data.txt -o out   → '-o'는 offset 가산, 출력지정 아님 → Usage 에러
#       xxd -d data.txt -i out   → '-i'는 C-include 덤프, 출력지정 아님 → Usage 에러
#       xxd -d data.txt out      → out에 'hexdump의 hexdump'가 써짐(방향 그대로) → file: "ASCII text"
#     → 정답은 방향(-r)이 핵심이고, 출력은 '위치 인자' 또는 redirect: xxd -r IN OUT  (또는 xxd -r IN > OUT)
# (3) gzip이 'unknown suffix' 거부 → gzip은 .gz 확장자를 요구 → mv로 붙여야 함
# (4) bzip2 함정: 'bzip2 file'은 기본 '압축'. 해제는 -d + .bz2 확장자. 또 'bzip2 -d'만 치면 stdin(터미널)을
#     읽으려 대기("I won't read compressed data from a terminal"). 출력 파일이 이미 있으면 덮어쓰기 거부 → rm 선행.
# ----------------------

# 핵심 1단계: hexdump → binary  (이번 세션 실제 입력: xxd -r data.txt data1.txt — 위치 인자로 출력)
...
> [!warning] bandit엔 binwalk/7z가 없다 — 그리고 설치도 못 한다
> 이번 세션에서 `sudo apt-get install binwalk`를 시도했으나 `sudo: sudo must be owned by uid 0 and have the setuid bit set`. bandit 계정은 **root 권한이 없고 sudo가 setuid로 설정돼 있지 않아** 패키지 설치 자체가 불가. 즉 이 Alternative는 *해당 도구가 이미 깔려 있을 때만* 유효하며, 실제 이 레벨은 기본 제공 도구(`xxd`/`file`/`gzip`/`bzip2`/`tar`)만으로 풀어야 한다. → 교훈: **환경에 없는 도구를 전제한 풀이는 계획이 아니다.** 먼저 `command -v binwalk`로 존재를 확인하라.
