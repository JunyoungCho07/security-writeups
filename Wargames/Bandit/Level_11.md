...
**Alternative 1**: 단일 tr (가장 정석) — *재풀이(2026-07-14) 세션에서 실제로 이걸 한 방에 씀*
```bash
tr 'A-Za-z' 'N-ZA-Mn-za-m' < data.txt
# 이번 세션 실제 입력 (SET 순서만 소문자 먼저, 등가):
cat data.txt | tr 'a-zA-Z' 'n-za-mN-ZA-M'
```
Trade-off: 파이프 1개·프로세스 1개. `< data.txt`로 UUOC(`cat` 남용)도 제거하면 더 깔끔(이번 세션은 `cat |`를 씀 — 결과 동일). 첫 풀이의 2단계·시도 1–4 삽질이 이번엔 단일 tr로 소거됨 = mastery 신호.
